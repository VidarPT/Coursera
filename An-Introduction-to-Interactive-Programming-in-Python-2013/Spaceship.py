...
class Ship:
    ...
    def rotation(self, keydown, clock_wise):
        if keydown:
            if clock_wise:
                self.angle_vel += 0.05
            else:
    ...
    def shoot(self):
        ...
        x_vel = self.vel[0] + angle_to_vector(self.angle)[0] * 10
        y_vel = self.vel[1] + angle_to_vector(self.angle)[1] * 10
        
        a_missile = Sprite([x_pos, y_pos], [x_vel, y_vel], 0, 0,
...
def rock_spawner():
    ...
    velocity = ((random.random() - random.random()) * 4,
                (random.random() - random.random()) * 4)
    angle = random.random() * math.pi*2 
    angle_vel = (random.random() - random.random()) * 0.1
    
    a_rock = Sprite(position, velocity, angle, angle_vel,
                    asteroid_image, asteroid_info)

def keydown(key):     
    if key == simplegui.KEY_MAP["left"]:
        my_ship.rotation(True, False)                
    
    if key == simplegui.KEY_MAP["right"]:
        my_ship.rotation(True, True)
...
def keyup(key):
    if key == simplegui.KEY_MAP["left"]:
        my_ship.rotation(False, False)                
    
    if key == simplegui.KEY_MAP["right"]:
        my_ship.rotation(False, True)
