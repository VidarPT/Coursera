...
namespace GameProject
{
    ...
    class NumberBoard
    {
            ...
            int correctNumber)
        {
			...
			// sideLength = (4 * BORDER_SIZE) + (3 * tileSideLength);
			tileSideLength = (sideLength - 4 * BORDER_SIZE) / 3;

			// initialize array of number tiles using nested for loops
			// one for the rows and the other for the columns
			// add a counter variable to draw correct number
			// for each tile
			int tileNumber = 1;
			for (int row = 0; row < NUM_ROWS; row++) {
				for (int column = 0; column < NUM_COLUMNS; column++) {
					tiles [row, column] = new NumberTile (contentManager,
						CalculateTileCenter (row, column), tileSideLength,
						tileNumber, correctNumber);
					tileNumber++;
				}
			}
        ...
        public bool Update(GameTime gameTime, MouseState mouse)
        {
			...
			//for (int row = 0; row < NUM_ROWS; row++) {
			//	for (int column = 0; column < NUM_COLUMNS; column++) {
			//		tiles [row, column].Update (gameTime, mouse);
			//	}
			//}
